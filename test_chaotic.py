...
def load_lorenz(id=0):
    ...
    flow_model.load_state_dict(torch.load(f'./results/lorenz/models/dynamics_model_{id}.pth'))
    flow_model.eval()
...
def perturb_traj(reference_trajs, f, tau=500, sigma=1e-5, eps=1e-8, dt=0.1):
    ...
    for i in range(num_simulation):
        ...
        x0 = traj_ref[:, 0]
        traj_perturbed = simulate_trajectory(f, x0 + dx, num_steps=tau, dt=dt).transpose(0, 1)
